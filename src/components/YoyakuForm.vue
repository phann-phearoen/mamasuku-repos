    ...
    <form @submit.prevent="submitForm">
      <form-slot :label="'お名前'">
        <template v-slot:required
          ><span style="color: red" v-if="!name.isValid"
            >（必須）</span
          ></template
        >
      ...
      <form-slot :label="'電話番号'">
        <template v-slot:required
          ><span style="color: red" v-if="!phone.isValid"
            >（必須）</span
          ></template
        >
        ...
        <q-input
          ...
          label="例) 070-1234-5678 (ハイフン含めて入力)"
      ...
      <form-slot :label="'メールアドレス'">
        <template v-slot:required
          ><span style="color: red" v-if="!email.isValid"
            >（必須）</span
          ></template
        >
      ...
      <form-slot :label="'ご希望の日時'">
        <template v-slot:required
          ><span style="color: red" v-if="!dateIsValid"
            >（必須）</span
          ></template
        >
...
export default {
  ...
  computed: {
    ...
    dateIsValid() {
      if (
        this.date1.day.isValid === false ||
        this.date1.timeStart.isValid === false ||
        this.date1.timeEnd.isValid === false
      ) {
        return false;
      } else return true;
    },
